...
const savedGameReducer: Reducer<AdminGameData | null, Actions> =
		...
		switch (action.type) {
			...
			case "heartbeat": {
				...
				if (
					...
					(state.timer as any)["endTime"] ==
						(timer as any)["endTime"] &&
					(state.timer as any)["timeLeft"] ==
						(timer as any)["timeLeft"]
				) {
					return state;
				}
