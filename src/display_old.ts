...
import { BaseCanvasItem } from "@/display_old/base";
import { Clock } from "@/display_old/clock";
import { HEIGHT, WIDTH } from "@/display_old/constants";
import { loadFonts } from "@/display_old/fonts";
import { PhaseTracker } from "@/display_old/phase";
import { TerrorTracker } from "@/display_old/terror";
import { Ticker } from "@/display_old/ticker";
import { TurnTracker } from "@/display_old/turn";
...
async function main(): Promise<void> {
	...
	if (module.hot) {
		module.hot.accept("@/display_old/clock", () => {
			clock = makeCanvas(Clock, clock);
		});
		module.hot.accept("@/display_old/turn", () => {
			turnTracker = makeCanvas(TurnTracker, turnTracker);
		});
		module.hot.accept("@/display_old/phase", () => {
			phaseTracker = makeCanvas(PhaseTracker, phaseTracker);
		});
		module.hot.accept("@/display_old/terror", () => {
			terrorTracker = makeCanvas(TerrorTracker, terrorTracker);
		});
		module.hot.accept("@/display_old/ticker", () => {
			ticker = makeCanvas(Ticker, ticker);
		});
		module.hot.accept(["@/display_old/constants", "@/display_old/fonts"]);
	}
